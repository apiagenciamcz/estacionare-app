...
export const handleCreateLogin = (email, password, confirmePassword) => (dispatch) => {
...
export const loginGoogleConfigure = () => (dispatch) => {
    ...
    GoogleSignin.configure({
        iosClientId: "307321601096-jpocbm5rtt137d4mgpkdi03troq82rb0.apps.googleusercontent.com", // only for iOS
        androidClientId: "307321601096-7o2jnm2kpgg3318ri9d1ok3qhinbbhnj.apps.googleusercontent.com" // only for ANDROID
    }).then (() => { 
        ...
        if(name.length){
            ...
            AsyncStorage.getItem('CurrentUser').then((userId) => {
                ...
                firebase.database().ref('users/'+ userId).update({
                    ...
                    name: name, 
                    cars: cars,
                    notifications: { beeps: false, vibration: false, parkingMeterAlert: false }
                }).then(() => {
                    dispatch(loading(false))
                    dispatch(
                        NavigationActions.navigate({
    ...
    setTimeout(() => {
        AsyncStorage.setItem('LoggedUser', 'true').then(() => {
            AsyncStorage.getItem('CurrentUser').then((userId) => {
                ...
                firebase.database().ref('users/'+ userId +'/account').push({
                    type: typeAccount,
                    status: 'valid',
                    date: {
                        first: now,
                        last: new Date(now.getFullYear(), now.getMonth() + 1, now.getDate()),
                    }
                }).then(() => {
                    dispatch(loading(false))
                    dispatch(

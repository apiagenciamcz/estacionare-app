...
export const getUser = () => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/name`).once("value").then((snapshot) =>  {
            dispatch({
                type: 'GET_NAME',
...
export const getFontSize = () => (dispatch) => {
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/fontSize`).once("value").then((snapshot) => {
            dispatch({
                type: 'GET_FONT_SIZE',
                payload: snapshot._value
            })
...
export const saveEditCar = (index, text) => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/cars/${index}`).update({
            name: text,
        }).then(() => {
            dispatch({
                type: 'ALTER_NAME'
...
export const deleteEditCar = (index) => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/cars/${index}`).remove()
...
export const addCar = () => (dispatch) =>{
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/cars`).push({
            name: '',
            value: ''
        }).then(() => {
            dispatch(getCarsUser())
...
export const getNotificationsUser = () => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/notifications`).once("value").then((snapshot) => {
            dispatch({
...
export const changeNotificationUser = (itemNotification, val) => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/notifications/`).update({
...
export const getCardsUser = () => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/cards`).once("value").then((snapshot) => {
            dispatch({
                type: 'GET_CARDS',
                payload: snapshot._value
            })
...
export const getlistReceipts = () => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        ...
        firebase.database().ref(`users/${userID}/receipts`).once("value").then((snapshot) => {
...
export const saveConfirmPassword = (confirmPassword) => (dispatch,getState) =>  {
    ...
    if(confirmPassword.length > 0){
        ...
        if(type === 'email'){
            dispatch(changeFirebaseEmail(confirmPassword, newValue.toString().trim()))
            dispatch({
...
export const cardEdit = (id) => (dispatch) => {
    ...
    if(id !== 'new'){
        ...
        AsyncStorage.getItem('CurrentUser').then((userID) => {
            firebase.database().ref(`users/${userID}/cards/${id}`).once("value").then((snapshot) => {
...
export const saveCard = (cards) => (dispatch) => {
    ...
    if(cards.num.length && cards.date.length && cards.cvv.length && cards.name.length){

        AsyncStorage.getItem('CurrentUser').then((userId) => {

            firebase.database().ref('users/'+ userId +'/cards/').push({
                cvv : cards.cvv,
                date : cards.date,
                flag : cards.flag,
                num : cards.num,
        ...
        setTimeout(() => {
            dispatch(Alert.alert(
                'Erro',
                'Por favor digite os dados do cartão'
            ))
        },100)
    }
}
...
export const updateCard = (cards) => (dispatch,getState) => {
    ...
    if(cards.num.length && cards.date.length && cards.cvv.length && cards.name.length){
        ...
        AsyncStorage.getItem('CurrentUser').then((userId) => {
            ...
            firebase.database().ref('users/' + userId + '/cards/' + key).update({
                cvv : cards.cvv,
                date : cards.date,
                flag : cards.flag,
                num : cards.num,
...
export const getMainCard = () => (dispatch) => {
    ...
    AsyncStorage.getItem('CurrentUser').then((userID) => {
        firebase.database().ref(`users/${userID}/mainCard`).once("value").then((snapshot) => {
            dispatch({
                type: 'GET_MAIN_CARD',
                payload: snapshot._value
            })
...
export const validateNumberCard = (value) => (dispatch) => {
    ...
    if(value.length == 19){
        ...
        if (!numberValidation.isPotentiallyValid) {
            ...
            dispatch(
                alertUser('Cartão invalido', 'Por favor, confira se os dados do cartão estão corretos')
            )
        }
...
export const validateDateCard = (value) => (dispatch) => {
    ...
    if(value.length == 5){
        ...
        if (!numberValidation.isPotentiallyValid) {
            ...
            dispatch(
                alertUser('Data invalida','Por favor, confira se os dados do cartão estão corretos')
            )
        }
...
export const validateCvvCard = (value) => (dispatch) => {
    ...
    if(value.length == 3){
        ...
        if (!numberValidation.isPotentiallyValid) {
            ...
            dispatch(
                alertUser('Código invalido', 'Por favor, confira se os dados do cartão estão corretos')
            )
        }

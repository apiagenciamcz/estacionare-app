                    ...
                    { 
                        Object.keys(partners).map((keyPartner) => 
                            Object.values(partners[keyPartner].areas).map((area, key) => 
                                <TouchableOpacity key={key} style={[styles.button, { backgroundColor: area.color }]} 
                                    onPress={() => this.props.setArea(area.name, area.value, keyPartner)} >
                                    <View>                        
...
export default connect(mapStateToProps, mapDispatchToProps )(PayStepThree)